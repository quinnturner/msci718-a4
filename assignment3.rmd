...
glm.probability = predict(glm.fit1, newdata = train, type = "response")
# Use these probabilities to predict red vs wine
glm.predict = ifelse(glm.probability > 0.5, "white", "red")
...
confMat <- table(glm.predict, trainOutcomeValues)
# Accuracy:
mean(glm.predict == trainOutcomeValues)

# Now let's compare this to our test data
glm.probability = predict(glm.fit1, newdata = test, type = "response")
# Use these probabilities to predict red vs wine
glm.predict = ifelse(glm.probability > 0.5, "white", "red")
# Lets use a confusion matrix to see how the logistic regression model performed
...
# Lets see our confusion matrix and accuracy values
confMat <- table(glm.predict, testOutcomeValues)
# Accuracy:
mean(glm.predict == testOutcomeValues)
...
### 4. Which factors are the biggest predictor of pH?
```{R}
